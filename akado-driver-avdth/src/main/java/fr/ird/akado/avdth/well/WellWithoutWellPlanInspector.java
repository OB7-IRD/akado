...
import static fr.ird.akado.avdth.Constant.CODE_WELL_NO_WELLPLAN;
import static fr.ird.akado.avdth.Constant.LABEL_WELL_NO_WELLPLAN;
...
/**
 * Check if the well plan is associated at one well existing.
 *
...
public class WellWithoutWellPlanInspector extends Inspector<Well> {
    ...
    public Results execute() {
        ...
        if (w.getWellPlans().isEmpty()) {
            WellResult r = new WellResult();
            r.set(w);
            r.setMessageType(Message.ERROR);
            r.setMessageCode(CODE_WELL_NO_WELLPLAN);
            r.setMessageLabel(LABEL_WELL_NO_WELLPLAN);
